...
func emit(event: String, ctx: Dictionary, gs: GameState, controller: GameController = null) -> Array:
	...
	for entry in _collect_sources(event, ctx, gs):
		...
		if not cost.is_empty():
			...
			if not CostCalculatorScript.can_afford(owner_pi, computed, gs):
				continue
...
func _prompt_optional(source: Variant, ab: Dictionary, ctx: Dictionary, gs: GameState) -> String:
	...
	gs.pending_prompt = {
		...
		"prompt": "[PROMPT] Optional ability — choose yes or no",
	}
