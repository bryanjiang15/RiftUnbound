...
def _profile() -> dict:
    """Minimal profile with one of each tunable weight group."""
    return {
        "schema_version": "2.0",
        "win_game": 1000.0,
        "state_weights": {
...
def _features(score_diff=0, might=0, game_over=False, bf=None, keyword_net=None):
    base = {
        "ai_index": 0,
        "game_over": game_over,
        "score_diff": score_diff,
        "unit_might_diff": might,
        "cards_in_hand_diff": 0,
        "runes_available_diff": 0,
...
def test_battlefield_control_value_signed_by_controller():
    ...
    feats = _features(bf={"battlefield-a": 0, "battlefield-b": 1})
...
def test_fit_detects_sign_flip_against_profile():
    """Profile says runes_available is negative; data says more runes -> wins."""
    prof = _profile()
    keys = tt.ordered_weight_keys(prof)
    idx = keys.index(("state_weights", "runes_available"))
    X, y = [], []
    for v in range(-6, 7):
        row = [0.0] * len(keys)
        row[idx] = float(v)
        X.append(row)
        y.append(1.0 if v > 0 else 0.0)  # positive runes correlate with winning
    fit = tt.fit_logistic(X, y, k=1.0, l2=0.01, lr=0.5, epochs=3000)
    raw = tt.raw_weights(fit, keys)
    diag = tt.diagnose(prof, raw, keys)
    runes = next(d for d in diag if d["key"] == "runes_available")
    assert runes["new"] > 0.0  # flipped from the profile's -0.1
    assert runes["status"] == "sign-flip"
